...
import { getServiceBySlug, getAllServices } from '../services/servicesService';
...
// Benefit icon pool — cycled by index
const BENEFIT_ICONS = [
...
// ─── Business Challenges by category ─────────────────────────────────────────
const CHALLENGES_BY_CATEGORY = {
  'Operations': [
    { title: 'Rising Operational Costs', body: 'Manual, fragmented processes drain resources and prevent teams from focusing on higher-value work that drives growth.' },
    { title: 'Inconsistent Customer Experience', body: 'Siloed channels and inconsistent service standards erode customer trust and increase churn in competitive markets.' },
    { title: 'Compliance & Risk Exposure', body: 'Evolving regulatory requirements and increasing audit scrutiny create operational risk without the right governance frameworks.' },
  ],
  'Digital & Technology': [
    { title: 'Legacy System Constraints', body: 'Outdated architectures limit agility, increase maintenance burden, and prevent adoption of modern capabilities.' },
    { title: 'Slow Time-to-Market', body: 'Lengthy development cycles and siloed engineering teams prevent organisations from responding to market opportunities.' },
    { title: 'Integration Complexity', body: 'Disconnected systems and data silos create friction across the business, undermining productivity and decision quality.' },
  ],
  'Data & AI': [
    { title: 'Untapped Data Assets', body: 'Vast volumes of enterprise data sit unused in disparate systems, leaving critical business insights undiscovered.' },
    { title: 'AI Adoption Barriers', body: 'Moving from AI experimentation to production-grade deployment at scale requires expertise most organisations lack internally.' },
    { title: 'Data Quality & Governance', body: 'Poor data quality and absence of governance frameworks undermine trust in analytics and create regulatory exposure.' },
  ],
  'Cloud & Infrastructure': [
    { title: 'Uncontrolled Cloud Spend', body: 'Without FinOps discipline and architecture governance, cloud costs escalate rapidly, eroding the expected ROI of migration.' },
    { title: 'Migration Risk & Complexity', body: 'Large-scale workload migration carries significant technical and business continuity risk without proven methodology.' },
    { title: 'Security & Compliance Gaps', body: 'Cloud environments introduce new attack surfaces and compliance obligations that require specialist expertise to manage.' },
  ],
};
const DEFAULT_CHALLENGES = [
  { title: 'Operational Inefficiency', body: 'Manual processes and disconnected systems create bottlenecks that limit throughput, raise costs, and frustrate customers.' },
  { title: 'Talent & Capability Gaps', body: 'Building and retaining specialist skills in-house is costly and slow — organisations need a partner who brings expertise on demand.' },
  { title: 'Pressure to Demonstrate ROI', body: 'Boards and investors demand clear, measurable returns from transformation investments with minimal tolerance for delay.' },
];
...
const WHY_US = [
  { icon: ICON_USERS,  heading: 'Deep Domain Expertise',  points: ['Sector-trained delivery teams', 'Industry-specific process frameworks', 'Regulatory and compliance fluency'] },
  ...
  { icon: ICON_SHIELD, heading: 'Security & Compliance',   points: ['ISO 27001 & SOC 2 Type II certified', 'GDPR and CCPA aligned processes', 'Regular third-party security audits'] },
...
function BenefitCard({ benefit, index }) {
  return (
    <div className="sdp-benefit-card">
      <div className="sdp-benefit-card__header">
        <div className="sdp-benefit-card__icon">
          {BENEFIT_ICONS[index % BENEFIT_ICONS.length]}
        </div>
        <span className="sdp-benefit-card__num">{String(index + 1).padStart(2, '0')}</span>
      </div>
      <h3 className="sdp-benefit-card__title">{benefit.title}</h3>
      <p className="sdp-benefit-card__body">{benefit.description}</p>
    </div>
  );
}

function OutcomeCard({ icon, title, body, metric }) {
  return (
    <div className="sdp-outcome-card">
      <div className="sdp-outcome-card__top">
        <div className="sdp-outcome-card__icon">{icon}</div>
        {metric && <span className="sdp-outcome-card__metric">{metric}</span>}
      </div>
      <h3 className="sdp-outcome-card__title">{title}</h3>
      <p className="sdp-outcome-card__body">{body}</p>
    </div>
  );
}
...
function ChallengeCard({ title, body, index }) {
  return (
    <div className="sdp-challenge-card">
      <span className="sdp-challenge-card__num">{String(index + 1).padStart(2, '0')}</span>
      <h3 className="sdp-challenge-card__title">{title}</h3>
      <p className="sdp-challenge-card__body">{body}</p>
    </div>
  );
}

function RelatedCard({ svc }) {
  return (
    <Link to={`/services/${svc.slug}`} className="sdp-related-card">
      <div className="sdp-related-card__body">
        <span className="sdp-related-card__category">{svc.category}</span>
        <h3 className="sdp-related-card__title">{svc.title}</h3>
        <p className="sdp-related-card__desc">{svc.description?.slice(0, 110)}…</p>
      </div>
...
export default function ServiceDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([svcRes, allRes]) => {
        ...
        const others = (allRes.data || []).filter((s) => s.slug !== slug);
        const same  = others.filter((s) => s.category === svcRes.data.category);
        const diff  = others.filter((s) => s.category !== svcRes.data.category);
    ...
    getCtaSection('service_detail')
      .then((res) => { if (res.data) setCta(res.data); })
      .catch(() => {});

    getHeroSection('service_detail')
  ...
  const challenges = CHALLENGES_BY_CATEGORY[service.category] || DEFAULT_CHALLENGES;
    ...
    <div className="sdp">
        ...
        <div className="sdp-section-container">
          <div className="sdp-challenges__layout">
            <div className="sdp-challenges__left">
              <span className="sdp-eyebrow">The Challenge</span>
              <h2 className="sdp-section-title">
                What Enterprises Struggle With
              </h2>
              <p className="sdp-section-sub">
                Before engaging us, clients in {service.category} consistently face the same critical obstacles. Recognise any of these?
              </p>
      ...
      <section className="sdp-outcomes">
        <div className="sdp-section-container">
          <div className="sdp-section-header">
            ...
            <p className="sdp-section-sub">
              Every engagement is designed around commercial outcomes, not just activities. Here is what clients consistently achieve.
            </p>
          </div>
        ...
        <div className="sdp-section-container">
          <div className="sdp-how__layout">
            <div className="sdp-how__left">
              ...
              <h2 className="sdp-section-title sdp-section-title--white">
                How We Deliver Results
              </h2>
              <p className="sdp-section-sub sdp-section-sub--white">
                A proven four-phase delivery model that takes you from current-state assessment through to sustained optimisation — with clear milestones at every stage.
              </p>
      ...
      {benefits.length > 0 && (
        <section className="sdp-benefits">
          <div className="sdp-section-container">
            <div className="sdp-section-header">
              <span className="sdp-eyebrow">Key Capabilities</span>
              <h2 className="sdp-section-title">What Our {service.title} Practice Delivers</h2>
              <p className="sdp-section-sub">
                Proven capabilities built over 4+ years of enterprise delivery, refined through multiple engagements.
              </p>
            </div>
      ...
      <section className="sdp-why">
        <div className="sdp-section-container">
          <div className="sdp-section-header sdp-section-header--white">
            ...
            <h2 className="sdp-section-title sdp-section-title--white">
              The Partner Enterprises Choose
            </h2>
            <p className="sdp-section-sub sdp-section-sub--white">
              4+ years of enterprise delivery experience, a focused delivery footprint, and an unwavering commitment to measurable outcomes.
            </p>
          </div>
